...
class MarstekAPI:
    ...
    def __init__(
        ...
        # Connection pooling and DNS optimization will be created lazily
    ...
    def _get_session(self) -> aiohttp.ClientSession:
        ...
        # Create connector lazily if not already created
        if self._connector is None:
            try:
                # Try to use AsyncResolver with reliable DNS servers
                resolver = aiohttp.AsyncResolver(nameservers=['8.8.8.8', '1.1.1.1'])
            except RuntimeError:
                # Fallback to default resolver if aiodns is not available
                resolver = None
            ...
            self._connector = aiohttp.TCPConnector(
                ...
                resolver=resolver  # Use resolver if available
            )
